...
export class Station extends TypedEmitter<StationEvents> {
    ...
    public processPushNotification(message: PushMessage): void {
        this.log.error(`Station.processPushNotification: ${message.device_sn} == ${this.getSerial()}, ${message.type}, ${message.event_type}`);
        if (message.type !== undefined && message.event_type !== undefined) {
            this.log.error(`Station.processPushNotification 1: ${message.device_sn} == ${this.getSerial()}, ${message.type}, ${message.event_type}`);
            if (message.event_type === CusPushEvent.MODE_SWITCH && message.station_sn === this.getSerial()) {
                this.log.error(`Station.processPushNotification 2: ${message.device_sn} == ${this.getSerial()}, ${message.type}, ${message.event_type}`);
                ...
                this.log.error(`Station.processPushNotification 3: ${message.device_sn} == ${this.getSerial()}, ${message.type}, ${message.event_type}`);
    ...
    public async setStatusLed(device: Device, value: boolean): Promise<void> {
        ...
        } else if (device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423 || (device.getDeviceType() === DeviceType.FLOODLIGHT && !device.isFloodLightT8420X())) {
            await this.p2pSession.sendCommandWithIntString({
                commandType: CommandType.CMD_DEV_LED_SWITCH,
                value: value === true ? 1 : 0,
                valueSub: device.getChannel(),
    ...
    public async setMotionDetection(device: Device, value: boolean): Promise<void> {
        ...
        if (device.isIndoorCamera() || (device.isFloodLight() && device.getDeviceType() !== DeviceType.FLOODLIGHT) || device.isFloodLightT8420X() || device.isWiredDoorbellT8200X() || device.isStarlight4GLTE()) {
            await this.p2pSession.sendCommandWithStringPayload({
                commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
                value: JSON.stringify({
    ...
    public async setMotionDetectionSensitivity(device: Device, value: number): Promise<void> {
        ...
        if ((device.isFloodLight() && device.getDeviceType() !== DeviceType.FLOODLIGHT) || device.isIndoorCamera() || device.isFloodLightT8420X()) {
            await this.p2pSession.sendCommandWithStringPayload({
                commandType: CommandType.CMD_DOORBELL_SET_PAYLOAD,
                value: JSON.stringify({
    ...
    public async setMotionDetectionType(device: Device, value: number): Promise<void> {
        ...
        if (device.isCamera2Product() || device.isBatteryDoorbell() || device.getDeviceType() === DeviceType.CAMERA ||
            ...
            device.getDeviceType() === DeviceType.FLOODLIGHT_CAMERA_8423 || device.isWiredDoorbellDual() || device.isStarlight4GLTE()) {
            await this.p2pSession.sendCommandWithInt({
